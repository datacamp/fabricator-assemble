...
var parseMaterials = function() {
  ...
  options.materials = typeof options.materials === "string"
    ? [options.materials]
    : options.materials;
  ...
  files.forEach(function(file) {
    var parent = getName(
      path.normalize(path.dirname(file)).split(path.sep).slice(-2, -1)[0],
      true
    );
    var collection = getName(
      path.normalize(path.dirname(file)).split(path.sep).pop(),
      true
    );
    ...
    if (isSubCollection) {
      assembly.materials[parent].items[collection] = assembly.materials[parent]
        .items[collection] || {
  ...
  files.forEach(function(file) {
    ...
    var parent = path
      ...
      .slice(-2, -1)[0];
    var isSubCollection = dirs.indexOf(parent) > -1;
    var id = isSubCollection
      ? getName(collection) + "." + getName(file)
      : getName(file);
    var key = isSubCollection
    ...
    var content = fileMatter.content.replace(
      /^(\s*(\r?\n|\r))+|(\s*(\r?\n|\r))+$/g,
      ""
    );
...
var registerHelpers = function() {
  ...
  Handlebars.registerHelper(
    ...
    function(name, context, opts) {
      ...
      var template = Handlebars.partials[key], fn;
...
var assemble = function() {
  ...
  files.forEach(function(file) {
    ...
    var pageMatter = getMatter(file), pageContent = pageMatter.content;
...
var createIndependentMaterialPreviews = function() {
  ...
  for (var material in materials) {
    ...
    var source = wrapPage(
      materialTemplate,
      assembly.layouts[options.materialPreviews || options.layout]
    ),
      context = assembly.materialData[material],
      template = Handlebars.compile(source);

    var copyPath = path.normalize(
      path.join(options.dest, options.materialPreviews, path.basename(material))
    );
    mkdirp.sync(path.dirname(copyPath));
